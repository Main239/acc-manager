...
Future<AccountFormData?> showAccountFormDialog({required BuildContext context, Account? existingAccount}) async {
  final accountNameController = TextEditingController(text: existingAccount?.accountName ?? '');
  final usernameController = TextEditingController(text: existingAccount?.username ?? '');
  final passwordController = TextEditingController(text: existingAccount?.password ?? '');
  final noteController = TextEditingController(text: existingAccount?.note ?? '');
  final isEditing = existingAccount != null;

  return showModalBottomSheet<AccountFormData>(context: context, isScrollControlled: true, useSafeArea: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(24))), builder: (bottomSheetContext) {
